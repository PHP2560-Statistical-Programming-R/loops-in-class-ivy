...
```{r}
for(rows.i in 1:nrow(x)){
  x[rows.i,] <- bubblesort(x[rows.i,])
}
...
if (x[1]==x[5]){
  ...
  hand="4ofKind"
} else if ((x[1]==x[2] & x[3]==x[5]) | (x[1]==x[3] & x[4]==x[5])) {
  hand="fullhouse"
} else if (x[1] == x[3] | x[2]==x[4] | x[3]==x[5]) {
  hand="3ofKind"
} else if (length(unique(x)) == 5 & (x[5] - x[1] == 4)) {
  hand="largestraight"
} else if (length(unique(x)) == 4 & (x[5] - x[1] == 3)) {
  hand="smallstraight"
} else hand="Chance"
...
if (x[i,1]==x[i,5]){
  hand[i]="Yahtzee"
} else if (x[i,1]==x[i,4] | x[i,2]==x[i,5]) {
  hand[i]="4ofKind"
} else if ((x[i,1]==x[i,2] & x[i,3]==x[i,5]) | (x[i,1]==x[i,3] & x[i,4]==x[i,5])) {
  hand[i]="fullhouse"
} else if (x[i,1] == x[i,3] | x[i,2]==x[i,4] | x[i,3]==x[i,5]) {
  hand[i]="3ofKind"
} else if (length(unique(x[i,])) == 5 & (x[i,5] - x[i,1] == 4)) {
  hand[i]="largestraight"
} else if (length(unique(x[i,])) == 4 & (x[i,5] - x[i,1] == 3)) {
  hand[i]="smallstraight"
} else hand[i]="Chance"
}
